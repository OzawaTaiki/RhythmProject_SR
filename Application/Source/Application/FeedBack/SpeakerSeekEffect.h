...
struct EllipticalPath
{
    ...
    Engine::Vector3 worldUp;         // ワールドの上方向
...
public:
    ...
    void Emit(const Engine::Vector3& startPos, const Engine::Vector3& targetPos, uint32_t count = 1);
    ...
    // 楕円軌道のパラメータ設定
    void SetWorldUp(const Engine::Vector3& up) { worldUp_ = up; }

private:
    /// <summary>
    /// 楕円軌道上の位置を計算
    /// </summary>
    Engine::Vector3 CalculateEllipticalPosition(const EllipticalPath& path) const;

    std::vector<Engine::Particle*> particles_;  // 生ポインタで保持（所有権はParticleSystemにある）
    std::vector<EllipticalPath> paths_;  // 各パーティクルの楕円軌道情報

    // デフォルト設定
    Engine::Vector3 worldUp_ = Engine::Vector3(0, 1, 0);  // ワールドの上方向
